...
def main():
    ...
    for i in tqdm(range(len(pt_data))):
        ...
        encoded_messages=''
        con_len=len(json_data_i['conversations'])
        for j in range(con_len-1):
            message = json_data_i['conversations'][j]
        ...
        instruct_i = encoded_messages.strip()
        output_i = PROMPT_RAW['ass'].format(response=json_data_i["conversations"][-1]['value'])

        whole_text = instruct_i + output_i
        direct_answer_text = 'Assistant: ' + output_i

        # instruct_i = json_data_i['conversations'][0]['value']
        # output_i = json_data_i['conversations'][1]['value']
        ...
        if args.max_length-instruct_i_len > 0:
            ...
            if len_1 <= 0 or len_2 <= 0:
                mean_rate_list.append((0,i))
                mean_list_1.append((0,i))
                mean_list_2.append((0,i))
                continue
    ...
    print('Do Rate')
    print(len(mean_rate_list), mean_rate_list[0][0])
    ...
    new_data = []
    for i, tmp in enumerate(json_data):
        tmp['ifd_score'] = mean_rate_list[i][0]
        new_data.append(tmp)
