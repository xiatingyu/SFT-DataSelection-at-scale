...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        for i in range(len(list_data_dict)):
            encoded_messages=''
            # encoded_messages+=PROMPT_RAW['system'].format(system=system)
            con_len=len(list_data_dict[i]['conversations'])
            for j in range(con_len-1):
                message = list_data_dict[i]['conversations'][j]
            ...
            response = PROMPT_RAW['ass'].format(response=list_data_dict[i]["conversations"][-1]['value'])
            sources.append(encoded_messages)
            targets.append(response+tokenizer.eos_token)
