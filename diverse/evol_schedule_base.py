...
class EvolSchedule:
    ...
    def train(self, rd):
        ...
        # if os.path.exists(rd_output_dir):
        #     rank0_print(f"*** Round {rd} ** Output-Dir Already Exists --> {rd_output_dir} ***")
        #     return
        # else:
    ...
    def get_embeddings_all_data(self, rd, use_model_path):
        ...
        return self.get_embeddings(rd=rd, data=all_data, use_model_path=use_model_path, requires_lc=False, requires_margin=False, requires_entropy=False)
    ...
    def get_embeddings_unlabeled_data(self, rd, use_model_path, requires_lc=False, requires_margin=False, requires_entropy=False):
        ...
        rank0_print(f'*** Round {rd} ** Trained Model loaded!')
        return self.get_embeddings(rd=rd, data=unlabeled_data, use_model_path=use_model_path, requires_lc=requires_lc, requires_margin=requires_margin, requires_entropy=requires_entropy)


    def process_chunk(self, device, data_chunk, use_model_path):
        print('distributed process_chunk...')
        last_hidden_states_avg_sub = []
        # torch.cuda.empty_cache()
        model = get_model(model_name_or_path=use_model_path, cache_dir=None)
        # model = AutoModelForCausalLM.from_pretrained(use_model_path)
        print('model loaded')
        model.to(device)
        model.eval()
        with torch.no_grad():
            for ids,datapoint in enumerate(data_chunk):
                input_ids = datapoint["input_ids"].unsqueeze(0).to(device)
                labels = datapoint["labels"].unsqueeze(0).to(device)
                ...
                last_hidden_states_avg_sub.append(last_hidden_states_avg.to('cuda:0')) 
                if ids % 10000 == 0:
                    print(f'chunk {ids} done, total {len(data_chunk)}')
        return last_hidden_states_avg_sub

    def get_embeddings(self, rd, data, use_model_path, requires_lc=False, requires_margin=False, requires_entropy=False):
        ...
        world_size = get_nproc_per_node()-1
        data_num = len(data["train_dataset"])
        
        data_per_proc = data_num // world_size + 1

        data_chunks = []
        for i in range(world_size):
            start = data_per_proc * i
            end = min(start + data_per_proc, data_num)
            data_tmp = list(data["train_dataset"])[start:end]
            data_chunks.append(data_tmp)
        

        devices = [f'cuda:{i+1}' for i in range(world_size)]

        with ThreadPoolExecutor(max_workers=world_size) as executor:
            last_hidden_states_avg = [
                executor.submit(self.process_chunk, device=devices[i], data_chunk=data_chunks[i], use_model_path=use_model_path)
                for i in range(world_size)
            ]
      
        for tmp in last_hidden_states_avg:
            last_hidden_states_avg_all.extend(tmp.result())
        
        # for item in last_hidden_states_avg:
        #     item.to('cuda:0')
        ...
        # with torch.no_grad():
        #     for _,datapoint in enumerate(tqdm(data["train_dataset"])):
        #         print(model.device)
