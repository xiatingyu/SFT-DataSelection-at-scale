...
def get_mbpp_dataset(data_dir: str,
                    tokenizer: PreTrainedTokenizerBase,
                    max_length: int,
                    use_chat_format: bool = True,
                    chat_format: str = "tulu",
                    **kwargs):
    """
    Get the mbpp dataset in the instruction tuning format. Each example is formatted as follows: 
    ...
    Returns:
        Dataset: The mbpp dataset containing input_ids, attention_mask, and labels.
    """
    file = f"{data_dir}/eval/mbpp/mbpp.test.jsonl"

    mbpp_few_shot_examples = []
    with open(file, 'r') as f:
        for line in f:
            mbpp_few_shot_examples.append(json.loads(line))

    dataset = {"input_ids": [], "attention_mask": [], "labels": []}

    for item in mbpp_few_shot_examples:
        
        question, answer = item['text'], item['code']
...
def get_dataset(task, **kwargs):
    ...
    if task == "bbh":
        ...
        return get_mbpp_dataset(**kwargs)
    else:
