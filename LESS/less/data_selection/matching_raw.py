...
N_SUBTASKS = {"mmlu": 57, "bbh": 27, "tydiqa": 9, "mbpp": 1}
...
for i, ckpt in enumerate(args.ckpts):
    ...
    training_info = None
    for sub_dir in range(len(os.listdir(path=ckpt_dir))):
...
influence_score = influence_score.reshape(
    influence_score.shape[0], N_SUBTASKS[args.target_task_names[0]], -1).mean(-1).max(-1)[0]
...
output_file = os.path.join(
    args.output_path, "influence_score.pt")
